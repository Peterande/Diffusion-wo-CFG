...
from models import ScaleAwareSiT
...
def euler_sampler(
    ...
    is_ScaleAware_model = isinstance(model, ScaleAwareSiT) or (hasattr(model, "module") and isinstance(model.module, ScaleAwareSiT))

    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            x_cur = x_next
            if cfg_scale > 1.0 and t_cur <= guidance_high and t_cur >= guidance_low and not is_ScaleAware_model:
                model_input = torch.cat([x_cur] * 2, dim=0)
                y_cur = torch.cat([y, y_null], dim=0)
            else:
                model_input = x_cur
                y_cur = y
            kwargs = dict(y=y_cur, s=cfg_scale) if is_ScaleAware_model else dict(y=y_cur)
            ...
            if cfg_scale > 1. and t_cur <= guidance_high and t_cur >= guidance_low and not is_ScaleAware_model:
